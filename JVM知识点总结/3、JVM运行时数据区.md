...
5. 《Java虚拟机规范》中对Java堆的描述是：**所有的对象实例以及数组都应当在运行时分配在堆上**。（The heap is the run-time data area from which memory for all class instances and arrays is allocated）
   - 从实际使用角度看：“几乎”所有的对象实例都在堆分配内存，但并非全部。因为还有一些对象是在栈上分配的（逃逸分析、标量替换等，不过这些其实很难触发）
...
1. **优先分配到Eden**
2. **大对象直接分配到老年代**：尽量避免程序中出现过多的大对象。开发中比较长的字符串或者数组，会直接存在老年代，但是因为新创建的对象都是朝生夕死的，所以这个大对象可能也很快被回收，但是因为老年代触发Major GC的次数比 Minor GC要更少，因此可能回收起来就会比较慢
3. **长期存活的对象分配到老年代**
...
1. **静态变量的存储位置变化：**
   ...
   - 静态变量不再存储在方法区，而是存储在堆中（具体由虚拟机实现决定）。
...
- 如果字符串常量池中不存在该字符串，则在常量池中创建一份，并返回此对象的地址
  ...
  >     （2）如果没有，把此对象放入串池（相当于在串池中new一个本对象），并返回串池中的对象地址
  >
  > - Jdk1.7起
  >    （1）如果串池中有，返回已有的串池中的对象的地址
  ...
  > java6
  >
  > <img src="http://jason243.online/JVM/jvm_java6.png" />
  >
  > Java8、11、17
  >
  ...
  > java6运行结果截图
  >
  > <img src="http://jason243.online/JVM/jvm_java6_002.png" />



### 7.5.2 字符串拼接的底层细节

#### 7.5.2.1 字符串变量拼接底层是StringBuilder()
...
public class StringIntern {
    public static void main(String[] args) {
        ...
        //String Table一个“1”，堆里面两个“1”，堆里面一个“11”
