...
> OopMap是针对寄存器和栈中的，指明有哪些引用
>
> 卡表是记忆集的卡精度，针对跨代引用的，主要在堆区
...
> 卡表（记忆集的卡精度）是为了记录跨代引用的，避免GC的时候扫描所有的对象寻找GC Roots，CMS因为只分年轻代和老年代，所以一个卡表就够了，而G1分了很多的Region，每个Region都可能是Eden、Survivor、Old或者Humongous，所以每一个Region都有一个属于自己的卡表，记录哪里有引用指向了自己。
